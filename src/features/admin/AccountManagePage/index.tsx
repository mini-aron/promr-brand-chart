...
export function AccountManagePage() {
  ...
  const handleAdd = useCallback(() => {
    ...
    const existingCode =
      accountCode && hospitals.some((h) => (h.accountCode ?? '') === accountCode);
    if (existingCode) {
  ...
  return (
    <div className={s.page}>
      <PageHeader title="거래처관리" description="거래처(병의원) 목록을 검색하고 확인합니다." />
        ...
        >
          <div className={s.modalBox} onClick={(e) => e.stopPropagation()}>
            <Row alignItems="center" justifyContent="space-between" className={s.modalHeaderWrap}>
              ...
              <Button variant="ghost" size="icon" onClick={closeAddModal} aria-label="닫기">
            ...
            {addError && <p className={s.addError}>{addError}</p>}
      ...
      />
      {filtered.length === 0 && <p className={s.emptyMessage}>조건에 맞는 거래처가 없습니다.</p>}
    </div>
  );
}
