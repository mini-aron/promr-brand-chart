...
export function AggregatePage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (effectiveCorporationId)
      list = list.filter((r) => r.corporationId === effectiveCorporationId);
  ...
  const settlementRows = useMemo(() => {
    const byHospital = new Map<string, { hospitalId: string; amount: number; itemCount: number }>();
  ...
  return (
    <div className={s.page}>
      ...
      <div className={s.aggregateFilterSection}>
      ...
      {settlementRows.length === 0 && (
        <p className={s.emptyMessage}>조건에 맞는 정산 실적이 없습니다.</p>
      )}
    </div>
  );
}
