...
import { Flex, Row } from '@/shared/components/ui/Flex';
...
export function SalesUploadPage() {
  ...
  const setSalesRows = useDemoPlayStore((s) => s.setSalesRows);
  const addSalesRows = useCallback(
    (rows: SalesRow[]) => {
  ...
  const [editedOverrides, setEditedOverrides] = useState<
    Record<string, { quantity?: number; productName?: string; hospitalId?: string }>
  >({});
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
  const [showConfirmModal, setShowConfirmModal] = useState(false);
  const previewSectionRef = useRef<HTMLDivElement>(null);

  const validProductCodes = useMemo(
    () => new Set(productFees.map((p) => p.productCode)),
    [productFees],
  );
  ...
  const setRowEdit = useCallback(
    (rowId: string, field: 'quantity' | 'productName' | 'hospitalId', value: number | string) => {
  ...
  const previewColumns = useMemo(
    () => [
      columnHelper.display({
        ...
        cell: ({ row }) =>
          row.original.type === 'data' ? (
            <span className={s.indexCell}>{(row.original.index as number) + 1}</span>
          ) : null,
      }),
      columnHelper.display({
        id: 'hospital',
      ...
      columnHelper.display({
        ...
        header: () => '사업자번호',
        cell: ({ row }) =>
          row.original.type === 'data' ? row.original.row.businessNumber || '-' : null,
      }),
      columnHelper.display({
        id: 'productName',
        header: () => '제품명',
  ...
  const handleDownloadProductFees = useCallback(() => {
    const rows: (string | number)[][] = [
      ['품목코드', '품목명', '수수료율(%)'],
      ...productFees.map((p) => [p.productCode, p.productName, p.feeRate]),
    ];
  ...
  const handleDownloadDummyExcel = useCallback(() => {
    const hospitalList = corporationHospitals.map((h) => ({
      name: h.name,
      businessNumber: h.businessNumber,
    }));
    const productNames = productFees.map((p) => p.productName);
  ...
  return (
    <div className={s.page}>
      ...
      <PageHeader
        title="실적 업로드"
        description="엑셀 파일을 업로드하여 판매 실적을 등록합니다."
      />
      ...
      <h2 className={s.sectionTitle1}>1. 엑셀 업로드</h2>
      <div
      ...
      {uploadedFiles.length > 0 && (
        <Row wrap="wrap" gap={8} alignItems="center" className={s.uploadedFilesWrap}>
          <span className={s.uploadedFilesLabel}>업로드된 파일:</span>
          {uploadedFiles.map((f, i) => (
            <span key={`${f.fileName}-${i}`} className="file-chip">
              <span className="name" title={f.fileName}>
                {f.fileName}
              </span>
              <Button
                variant="ghost"
      ...
      {preview.length > 0 && (
        <div ref={previewSectionRef} className={s.previewSection}>
          <h2 className={s.previewSectionTitle}>2. 업로드 결과 확인</h2>
          ...
          {(invalidHospitalCount > 0 || invalidProductCodeCount > 0) && (
            <div className={s.invalidWarning}>
              ...
              {invalidHospitalCount > 0 &&
                ` ${invalidHospitalCount}건의 병의원 사업자번호가 등록된 거래처와 일치하지 않습니다. 병의원 셀을 확인하세요.`}
              {invalidProductCodeCount > 0 &&
                ` ${invalidProductCodeCount}건의 제품코드가 품목 마스터에 없습니다. 제품코드 셀을 확인하세요.`}
            </div>
          )}
                  ...
                  return (
                    <tr key={row.id}>
                      {row.getVisibleCells().map((cell) => {
                        ...
                        const dataInvalid =
                          raw.type === 'data' &&
                          ((isHospital && raw.isInvalidHospital) ||
                            (isProductCode && raw.isInvalidProductCode));
                        return (
                          <td
                            key={cell.id}
          ...
          <Row gap={8} alignItems="center" className={s.rowMarginTop}>
            <Button variant="primary" onClick={openConfirmModal}>
              실적 등록
            </Button>
            <span className={s.totalCountText}>전체 {preview.length}건</span>
          </Row>
        </div>
      )}
