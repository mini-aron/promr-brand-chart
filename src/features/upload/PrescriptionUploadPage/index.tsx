...
export function PrescriptionUploadPage() {
  ...
  const setPrescriptionUploads = useDemoPlayStore((s) => s.setPrescriptionUploads);
  const hospitals = useDemoPlayStore((s) => s.hospitals);
  const salesRows = useDemoPlayStore((s) => s.salesRows);
  const addPrescriptionUpload = useCallback(
  ...
  const submitUpload = useCallback(() => {
    if (files.length === 0 || targetSalesRows.length === 0) {
      setMessage(
        '이미지를 선택하고, 해당 실적이 존재하는지 확인해 주세요. (병의원·월에 맞는 실적이 있어야 합니다.)',
      );
      return;
    }
    ...
    setMessage(
      `처방 사진 ${files.length}장이 업로드되었습니다. (${hospitalId ? '병의원별' : '전체'})`,
    );
    clearPreviews();
  }, [
    files.length,
  ...
  return (
    <div className={s.page}>
      ...
      <PageHeader
        title="처방사진 업로드"
        description="대상(월 · 병의원)을 선택한 뒤, 해당 실적에 대한 처방 사진을 업로드합니다."
      />

      <section className={s.selectCard}>
        <h2 className={s.sectionTitle}>1. 대상 선택</h2>
        ...
        <p className={s.targetCount}>
          해당 조건의 실적:{' '}
          <strong className={s.targetCountStrong}>{targetSalesRows.length}건</strong>
        </p>
      </section>

      <section className={s.sectionWrap}>
        <h2 className={s.sectionTitle2}>2. 처방 사진 업로드</h2>
        ...
        <div
          ...
          onDragOver={(e) => {
            e.preventDefault();
            if (canUploadImages) setIsDrag(true);
          }}
      ...
      <Button
        ...
        disabled={!canUploadImages || files.length === 0}
      >
        처방사진 등록
      </Button>
    </div>
