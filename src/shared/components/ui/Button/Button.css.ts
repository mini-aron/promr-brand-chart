...
const base = style({
  ...
  gap: 6,
  transition:
    'background-color 0.15s, border-color 0.15s, box-shadow 0.15s, opacity 0.15s, transform 0.1s',
  selectors: {
    '&:disabled': {
      opacity: 0.6,
      cursor: 'not-allowed',
...
export const variantGhost = style({
  ...
  selectors: {
    ...
    '&:active:not(:disabled)': {
      backgroundColor: 'color-mix(in srgb, var(--color-text) 14%, transparent)',
    },
  },
});
