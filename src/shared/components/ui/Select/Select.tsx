...
export function getItemClass(isSelected: boolean, isFocused: boolean): string {
...
type BaseProps = {
  ...
  /** `above`: 목록을 트리거 위로 열어 하단 overflow에 잘리지 않게 함 */
...
function SelectCore({
  ...
  const handleListClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (!li || multiple) return;
      const idx = parseInt(li.dataset.optionIndex ?? '', 10);
      const value = idx === -1 ? searchTerm : filteredOptions[idx]?.value;
      if (value === undefined) return;
      onChange([value]);
      closeDropdown();
    },
    [multiple, searchTerm, onChange, closeDropdown, filteredOptions],
  );
  ...
  return (
    ...
    >
      <div
        ...
        className={clsx(
          s.trigger,
          size === 'small' && s.triggerSmall,
          size === 'large' && s.triggerLarge,
        )}
        onClick={() => setIsOpen((p) => !p)}
      >
        <span>{displayLabel}</span>
        {isOpen ? (
          <ChevronUp size={ICON_SIZE} aria-hidden />
        ) : (
      ...
      {isOpen && (
        <ul
          ref={listRef}
          className={clsx(
            enableSearch ? s.listWithSearch : s.list,
            menuPlacement === 'above' && s.listAbove,
          )}
          role="listbox"
...
export function MultipleSelect(props: MultipleSelectProps) {
  ...
  return (
    <SelectCore {...rest} value={selectedItems} onChange={onChange} onRemove={onRemove} multiple />
  );
}
